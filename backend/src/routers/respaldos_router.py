...
def _load_config() -> dict | None:
    """Read S3 backup config from environment variables. Returns None if incomplete.
    Uses S3_BACKUP_* variables (separate from colposcopy S3 credentials)."""
    config = {
        "AWS_ACCESS_KEY_ID": os.environ.get("S3_BACKUP_ACCESS_KEY_ID", ""),
        "AWS_SECRET_ACCESS_KEY": os.environ.get("S3_BACKUP_SECRET_ACCESS_KEY", ""),
        "S3_BACKUP_BUCKET": os.environ.get("S3_BACKUP_BUCKET", ""),
        "S3_BACKUP_PREFIX": os.environ.get("S3_BACKUP_PREFIX", ""),
        "AWS_DEFAULT_REGION": os.environ.get("S3_BACKUP_REGION", "us-east-1"),
    }
...
def get_status(_user=Depends(get_current_user)):
    ...
    if not config:
        for var in ["S3_BACKUP_ACCESS_KEY_ID", "S3_BACKUP_SECRET_ACCESS_KEY", "S3_BACKUP_BUCKET", "S3_BACKUP_PREFIX"]:
...
def create_backup(_user=Depends(get_current_user)):
    ...
    config = _load_config()
    if not config:
        raise HTTPException(status_code=400, detail="Backup S3 no configurado. Configura las variables en .env")
    ...
    filename = f"expediente_ginecologia_{now.strftime('%Y-%m-%d_%H%M%S')}.db"
