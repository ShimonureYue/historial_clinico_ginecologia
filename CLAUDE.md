# CLAUDE.md - Expediente Clínico (Ginecología)
...
- **Auth:** JWT with python-jose, 24h tokens, granular permissions per module
- **Images:** AWS S3 with presigned URLs for colposcopy photos
- **PDFs:** jsPDF (frontend prescription), ReportLab (backend colposcopy report)
...
## Build & Deploy
```bash
python scripts/build_package.py  # Creates build/ExpedienteClinico/
```
